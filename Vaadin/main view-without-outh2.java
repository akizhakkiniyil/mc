...
import com.vaadin.flow.component.notification.Notification; // Keep Notification import
...
public class MainView extends VerticalLayout {
    ...
    public MainView(AccessGovernanceClientService clientService) { // Removed AuthenticationContext injection
        ...
        setPadding(true);
        setSpacing(true);
        // Removed LumoUtility.Padding.MEDIUM, LumoUtility.Gap.MEDIUM as they are not defined without Lumo theme
        // If you use Lumo theme, you can keep these.
        ...
        // anomalyEmployeeId.setReadOnly(true); // No longer read-only
            ...
            clientService.explainPolicyDrift(groupId, baselineDate)
                  ...
                  .subscribe(
                            drift -> ui.access(() -> { // Use ui.access for UI updates
                                try {
                                    setResponse(objectMapper.writeValueAsString(drift));
                                } catch (Exception e) {
                                    setResponse("Error formatting response: " + e.getMessage());
                                }
                            }),
